...
class MimirCoordinatorK8SOperatorCharm(ops.CharmBase):
    ...
    def _rules_sync_command(self, rules_file_paths: List[str]) -> List[str]:
        ...
        When Mimir is served over TLS, ``mimirtool`` must be told which CA to trust; otherwise
        ``rules sync`` fails certificate verification against Mimir's (internally-issued) certificate
        and alert rules are never loaded. The CA bundle is guaranteed to be on disk whenever
        ``internal_url`` is https (see ``are_certificates_on_disk``).
        ...
        if self.internal_url.startswith("https"):
            command.append(f"--tls-ca-path={CA_CERT_PATH}")
        return command
